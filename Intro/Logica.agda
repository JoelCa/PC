...
∨∧→ pq = (λ p → pq (left p) ) , (λ q → pq (right q))
...
deMorgan¬∧¬ : {P Q : prop} → (¬ P) ∧ (¬ Q) → ¬ (P ∨ Q)
deMorgan¬∧¬ (x , y) (left p) = x p
deMorgan¬∧¬ (x , y) (right q) = y q
  
deMorgan¬∨¬ : {P Q : prop} → (¬ P) ∨ (¬ Q) → ¬ (P ∧ Q)
deMorgan¬∨¬ (left p) (x , y) = p x
deMorgan¬∨¬ (right q) (x , y) = q y
...
-- Agregado por mi: creo que NO se puede probar.
-- 2curry∀← : {A : Set}{P : A → Set}
--          → ((a : A) → P a) → (∃ A P)
-- 2curry∀← f = {!!} , {!!}
...
¬¬terex : {P : prop} → ¬¬ (P ∨ ¬ P)
¬¬terex = λ x → x (right (λ x₁ → x (left x₁)))
...
TerEx→RND : TerEx → RAA
TerEx→RND x = λ {P} x₁ → case (λ x₂ → x₂) (λ x₂ → efq (x₁ x₂)) (x {P})

ret¬¬ : {P : prop} → P → ¬¬ P
ret¬¬ = pnnp

bind¬¬ : {P Q : prop} → ¬¬ P → (P → ¬¬ Q) → ¬¬ Q 
bind¬¬ x y z = x (λ x₁ → y x₁ z)

map¬¬ : {P Q : prop} → ¬¬ P → (P → Q) → ¬¬ Q
map¬¬ x y z = x (λ x₁ → z (y x₁))

app¬¬ : {P Q : prop} → ¬¬ (P → Q) → ¬¬ P → ¬¬ Q
app¬¬ x y z = x (λ x₁ → y (λ x₂ → z (x₁ x₂)))

∧¬¬-1 : {P Q : prop} → ¬¬ (P ∧ Q) → ¬¬ P ∧ ¬¬ Q
∧¬¬-1 x = (λ x₁ → x (λ x₂ → x₁ (fst x₂))) , (λ x₁ → x (λ x₂ → x₁ (snd x₂)))

∧¬¬-2 : {P Q : prop} → ¬¬ P ∧ ¬¬ Q → ¬¬ (P ∧ Q) 
∧¬¬-2 (p , q) pq = p (λ x → q (λ y → pq (x , y)))
...
∨¬¬-2 : {P Q : prop} → ¬¬ P ∨ ¬¬ Q → ¬¬ (P ∨ Q) 
∨¬¬-2 (left p) = λ x → p (λ y → x (left y))
∨¬¬-2 (right q) = λ x → q (λ y → x (right y))
...
¬¬deMorgan¬∧ : {P Q : prop} → ¬ (P ∧ Q) → ¬¬ ((¬ P) ∨ (¬ Q))
¬¬deMorgan¬∧ x y = y (left (λ p → y (right (λ q → x (p , q)))))
